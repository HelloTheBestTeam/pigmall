...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		...
		/*
		 * clients.inMemory() .withClient("XcWebApp")//客户端id
		 * .secret("XcWebApp")//密码，要保密 .accessTokenValiditySeconds(60)//访问令牌有效期
		 ...
		 * "refresh_token", "password") .scopes("app");//客户端范围，名称自定义，必填
		 */
	}
	...
	// RedisTokenStore redisTokenStore = new
	...
	public JwtAccessTokenConverter jwtAccessTokenConverter(
		...
		DefaultAccessTokenConverter accessTokenConverter = (DefaultAccessTokenConverter) converter
	...
	public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
		 ...
		 * tokenEnhancerChain.setTokenEnhancers(new
		 * ArrayList<>(tokenEnhancers)); DefaultTokenServices
		 ...
		 * defaultTokenServices.setTokenEnhancer(tokenEnhancerChain); endpoints
		 * .authenticationManager(authenticationManager)
		 * .userDetailsService(userDetailsService) //.tokenStore(tokenStore);
		 * .tokenServices(defaultTokenServices);
		 */
		endpoints.accessTokenConverter(jwtAccessTokenConverter).authenticationManager(authenticationManager)// 认证管理器
	...
	public void configure(AuthorizationServerSecurityConfigurer oauthServer) throws Exception {
		// oauthServer.checkTokenAccess("isAuthenticated()");//校验token需要认证通过，可采用http
		// basic认证
